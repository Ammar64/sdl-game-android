...
void init_players_tab() {
    ...
    for (i = 0; i < MAX_PLAYERS; i++) {
        players_server[i].position.w = GAME_PLAYER_SIZE;
        players_server[i].position.h = GAME_PLAYER_SIZE;
    ...
    while (1) {
        ...
        if (its_an_old_client(client_pos)) {
            ...
            if(players_server[client_pos].shoot && !players_server[client_pos].reloading) {
                ...
                temp.position.w = GAME_BULLET_SIZE;
                temp.position.h = GAME_BULLET_SIZE;
                temp.face = players_server[client_pos].face;
                if (temp.face == 1) {
                    temp.position.x += GAME_PLAYER_SIZE;
                } else {
                    temp.position.x -= GAME_BULLET_SIZE;
                }
